...
import com.wingify.utils.EventUtil;
...
public class WingifyClient {
    private Settings processedSettings;
    public String settings;
    private WingifyInitOptions options;
    ...
    public WingifyClient(String settings, WingifyBuilder wingifyBuilder) {
        try {
            this.options = wingifyBuilder.options;
    ...
    protected void sendSdkInitAndUsageStatsEvent(long settingsInitTime) {
        try {
            ...
            if (sdkMetaInfo == null || (sdkMetaInfo.get("wasInitializedEarlier") == null || !sdkMetaInfo.get("wasInitializedEarlier").equals(true))) {
                ...
                if (this.wingifyBuilder.getSettingsService().isSettingsValidOnInit) {
                    // send the sdk init event
                    EventUtil.sendSdkInitEvent(this.wingifyBuilder.getSettingsManager(), this.wingifyBuilder.getSettingsManager().settingsFetchTime, settingsInitTime, EventEnum.WINGIFY_SDK_INIT_EVENT.getValue());
                }
            }
            ...
            wingifyBuilder.getLoggerService().log(LogLevelEnum.ERROR, "SDK_INIT_EVENT_FAILED", new HashMap<String, Object>() {{
    ...
    public GetFlag getFlag(String featureKey, WingifyUserContext context) {
        ...
        String uuid = null;
        try {
            wingifyBuilder.getLoggerService().log(LogLevelEnum.DEBUG, "API_CALLED", new HashMap<String, Object>() {{
                put("apiName", apiName);
            }});
            ...
            if (!DataTypeUtil.isNull(context.getBucketingSeed())) {
                if (!DataTypeUtil.isString(context.getBucketingSeed())
                    ...
                    wingifyBuilder.getLoggerService().log(LogLevelEnum.ERROR, "INVALID_BUCKETING_SEED", new HashMap<String, Object>() {{
                    }});
            ...
            ServiceContainer serviceContainer = new ServiceContainer(context.getId(), this.wingifyBuilder.getLoggerService(), this.wingifyBuilder.getSettingsManager(), this.options, wingifyBuilder.getBatchEventQueue(), this.processedSettings);
            ...
            wingifyBuilder.getLoggerService().log(LogLevelEnum.ERROR, "EXECUTION_FAILED", new HashMap<String, Object>() {{
                put("apiName", "getFlag");
                put("err", exception.getMessage());
     ...
     */
    private Map<String, Boolean> track(String eventName, WingifyUserContext context, Map<String, ?> eventProperties) {
        ...
        try {
            ...
            if (!DataTypeUtil.isString(eventName)) {
                wingifyBuilder.getLoggerService().log(LogLevelEnum.ERROR, "INVALID_PARAM", new HashMap<String, Object>() {{
                    put("apiName", apiName);
                    put("key", "eventName");
     ...
     * @return Map containing the event name and its status
     */
    public Map<String, Boolean> trackEvent(String eventName, WingifyUserContext context, Map<String, ?> eventProperties) {
     ...
     * @return Map containing the event name and its status
     */
    public Map<String, Boolean> trackEvent(String eventName, WingifyUserContext context) {
     ...
     * @param context User context
     */
    public void setAttribute(Map<String, Object> attributeMap, WingifyUserContext context) {
     ...
     * @param context User context
     */
    public void setAttribute(String key, Object value, WingifyUserContext context) {
    ...
    public boolean flushEvents() {
        ...
        if (this.batchEventQueue != null) {
            ...
            wingifyBuilder.getLoggerService().log(LogLevelEnum.DEBUG, String.format(
            ...
            wingifyBuilder.getLoggerService().log(LogLevelEnum.ERROR, "BATCHING_NOT_ENABLED", new HashMap<String, Object>() {{
     ...
     /**
     * This method is used to update the settings on the WingifyClient instance
     * It validates the new settings and updates the processedSettings
     * @param newSettings New settings to be updated
     */
    private void updateSettingsOnWingifyClient(String newSettings) {
        try {
    ...
    private String updateSettings(String settings, Boolean isViaWebhook) {
        ...
        try {
            ...
            if (settings == null || settings.isEmpty()) {
                wingifyBuilder.getLoggerService().log(LogLevelEnum.ERROR, "INVALID_SETTINGS_SCHEMA", new HashMap<String, Object>() {{
            ...
            // Update the settings on the WingifyClient instance
            this.updateSettingsOnWingifyClient(settings);
            wingifyBuilder.getLoggerService().log(LogLevelEnum.INFO, "SETTINGS_UPDATED", new HashMap<String, Object>() {{
                put("brand", LogMessageUtil.getBrand(options.getIsViaVWO()));
                put("apiName", apiName);
                put("isViaWebhook", isViaWebhook.toString());
            }});
            return settings;
        } catch (Exception exception) {
            wingifyBuilder.getLoggerService().log(LogLevelEnum.ERROR, "UPDATING_CLIENT_INSTANCE_FAILED_WHEN_WEBHOOK_TRIGGERED", new HashMap<String, Object>() {{
                put("brand", LogMessageUtil.getBrand(options.getIsViaVWO()));
                put("apiName", apiName);
                put("err", exception.getMessage());
                put("isViaWebhook", isViaWebhook.toString());
    ...
    public String updateSettings(Boolean isViaWebhook) {
        ...
        try {
            ...
            this.settings = this.wingifyBuilder.getSettingsManager().fetchSettings(isViaWebhook);
    ...
    public Boolean setAlias(String userId, String aliasId) {
        ...
        try {
            ...
            if (!wingifyBuilder.getSettingsManager().isGatewayServiceProvided) {
            ...
            // create Service Container instance
            ServiceContainer serviceContainer = new ServiceContainer(userId, this.wingifyBuilder.getLoggerService(), this.wingifyBuilder.getSettingsManager(), this.options, wingifyBuilder.getBatchEventQueue(), this.processedSettings);
     ...
     * @return Boolean value indicating if the alias was set successfully
     */
    public Boolean setAlias(WingifyUserContext context, String aliasId) {
     ...
     */
    private String getUUIDFromContext(WingifyUserContext context, String apiName) {
        if (this.processedSettings.isWebConnectivityEnabled()) {
            ...
            if (UUIDUtils.isWebUuid(context.getId())) {
                ...
                wingifyBuilder.getLoggerService().log(LogLevelEnum.DEBUG, "WEB_UUID_FOUND", new HashMap<String, Object>() {{
                    put("brand", LogMessageUtil.getBrand(options.getIsViaVWO()));
                    put("apiName", apiName);
